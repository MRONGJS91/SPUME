...
class BiasedDataset(Dataset):
    def __init__(self, basedir, split="train", transform=None, concept_embed=None):
        ...
        if not os.path.exists(metadata_path):
            metadata_path = os.path.join(os.path.dirname(basedir), "metadata.csv")
        if not os.path.exists(metadata_path):
            raise FileNotFoundError(f"metadata.csv not found in {basedir} or its parent")
    ...
    def __getitem__(self, idx):
        ...
        candidates = [
            ...
            os.path.join(os.path.dirname(self.basedir), filename),
            os.path.join(self.basedir, "images", filename),
            os.path.join(os.path.dirname(self.basedir), "images", filename),
        ]
        img_path = next((p for p in candidates if os.path.exists(p)), None)
        if img_path is None:
            raise FileNotFoundError(
                f"Image file not found for {filename}. Tried: {candidates}"
            )
