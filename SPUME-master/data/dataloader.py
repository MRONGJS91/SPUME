...
from data.spawrious_data import SpawriousDataset
...
def get_metashift_loader(
    ...
    """Build data loaders for a MetaShift dataset.

    Follows the same pattern as ``get_spawrious_loader``.
    """
    train_transform = get_transform_biased(
        target_resolution=(224, 224), train=True, augment_data=True
    )
...
def get_loader(args, batch_sampler=None):
    ...
    if args.dataset.startswith("spawrious_o2o_"):
        train_loader, idx_train_loader, val_loader, test_loader = get_spawrious_loader(
            args.data_folder,
            args.metadata_path,
            args.augmentation,
            args.batch_size,
    ...
    if args.dataset.startswith("metashift"):
        train_loader, idx_train_loader, val_loader, test_loader = get_metashift_loader(
            args.data_folder,
            args.metadata_path,
            args.augmentation,
            args.batch_size,
