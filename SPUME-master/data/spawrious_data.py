...
def infer_embedding_key_path(concept_embed):
    embed_path = Path(concept_embed)
...
class SpawriousDataset(Dataset):
    def __init__(
        self,
        data_root,
        metadata_path,
        ...
        self.metadata_df = metadata_df[split_mask].reset_index(
            drop=True
        )
        ...
        self.img_key_array = np.array(
            [normalize_image_key(path) for path in self.img_path_array]
        )

        self.n_classes = np.unique(self.y_array).size
        env_names = sorted(self.metadata_df["env"].unique())
        self.env_name_to_idx = {name: idx for idx, name in enumerate(env_names)}
        self.p_array = self.metadata_df["env"].map(self.env_name_to_idx).astype(int).values
        ...
        if concept_embed and Path(concept_embed).exists() and split != "test":
            ...
            key_path = infer_embedding_key_path(concept_embed)
            if key_path.exists():
                with open(key_path, "rb") as f:
                    embedding_keys = [normalize_image_key(k) for k in pickle.load(f)]
                if len(embedding_keys) != len(all_embeddings):
                    raise ValueError(
                        f"concept key count ({len(embedding_keys)}) does not match "
                        f"embedding rows ({len(all_embeddings)}) for {concept_embed}"
                    )
                if len(embedding_keys) != len(set(embedding_keys)):
                    raise ValueError(f"duplicate concept keys found in {key_path}")
                key_to_row = {key: row_idx for row_idx, key in enumerate(embedding_keys)}
                missing = [key for key in self.img_key_array if key not in key_to_row]
                if missing:
                    raise KeyError(
                        f"{len(missing)} {split} images are missing concept embeddings; "
                ...
                non_test_mask = split_mask[split_mask_non_test].to_numpy()
                self.embeddings = all_embeddings[non_test_mask]
        else:
...
def main():
    parser = argparse.ArgumentParser(description="Minimal test for SpawriousDataset.")
    parser.add_argument(
        "--data-root",
