...
def prepare_model(dataset, backbone, pretrained=True):
    ...
    if dataset.startswith("spawrious_o2o_"):
        model = ERMCosineModel(backbone, 4, pretrained)
    if dataset.startswith("metashift"):
        # MetaShift can be multi-class; n_classes is set from config or auto-detected.
        # The caller should set model.n_classes = args.n_classes if provided.
        model = ERMCosineModel(backbone, 2, pretrained)  # default: binary (Cat vs Dog)
    model.cuda()
    return model
...
def pretrain(args):
    ...
    host_name = platform.node() or os.environ.get("COMPUTERNAME", "host")
    output_dir = os.path.join(
        args.save_folder,
        f"pretrain_cosine_{args.dataset}_{args.backbone}_{timestamp}_{host_name}",
    )
    ...
    train_method = getattr(args, "train_method", "erm").lower()
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch in tqdm.tqdm(train_loader, leave=False):
            ...
            if train_method == "mixup":
                ...
                logits = model(x)
                loss = mixup_criterion(criterion, logits * args.temp, y_a, y_b, lam)
            else:
                logits = model(x)
